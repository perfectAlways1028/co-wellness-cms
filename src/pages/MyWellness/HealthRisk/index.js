...
class HealthRisk extends Component {
    ...
    onAddNew = () => {
        this.props.history.push('/main/my-wellness/healthRisk/add');
    }
    onRowClick = (rowData, index) => {
        let id = rowData.id;
        this.props.history.push(`/main/my-wellness/healthRisk/detail/${id}`);
    ...
    onApplyFilter = () => {
        ...
        this.props.healthRiskStore.findHealthRisks(filter)
    }
    onClear = () => {
        this.setState({
            query: null,
    ...
    render () {
        const { totalCount, healthRisks } = this.props.healthRiskStore;
        let data = healthRisks;
        return <div className="cw-sub-wellness-container">  
            <div className="sub-header">
                <div className="sub-title">{Strings.txtHealthRiskList}</div>
                <div className="buttons">
                    <CButton 
                        onClick={()=>{this.onAddNew()}}
                        type="secondary"
                        containerStyle={{width: '172px', height: '40px', marginRight: '32px'}}>{Strings.txtBtnAddNewHealthRisk}</CButton>
                </div>
            </div>
            <div className="sub-filter">
                <div className="cw-row">
                    <CTextInput type='search' 
                        placeholder={Strings.txtHealthRiskSearchPlaceholder} 
...
export default inject('healthRiskStore')(observer(HealthRisk));
