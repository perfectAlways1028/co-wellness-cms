...
class Meal extends Component {
    ...
    onAddNew = () => {
        this.props.history.push('/main/my-wellness/meal/add');
    }
    onRowClick = (rowData, index) => {
        let id = rowData.mealID;
        this.props.history.push(`/main/my-wellness/meal/detail/${id}`);
    ...
    onApplyFilter = () => {
        ...
        this.props.mealStore.findMeals(filter)
    }
    onClear = () => {
        this.setState({
            query: null,
    ...
    render () {
        const { totalCount, meals } = this.props.mealStore;
        let data = meals;
        return <div className="cw-sub-wellness-container">  
            <div className="sub-header">
                <div className="sub-title">{Strings.txtMealList}</div>
                <div className="buttons">
                    <CButton 
                        onClick={()=>{this.onAddNew()}}
                        type="secondary"
                        containerStyle={{width: '172px', height: '40px', marginRight: '32px'}}>{Strings.txtBtnAddNewMeal}</CButton>
                </div>
            </div>
            <div className="sub-filter">
                <div className="cw-row">
                    <CTextInput type='search' 
                        placeholder={Strings.txtMealSearchPlaceholder} 
...
export default inject('mealStore')(observer(Meal));
