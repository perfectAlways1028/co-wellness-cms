...
const fields = [
    {
        title: Strings.txtFieldSymptomID,
        field: 'symptomsID'
    },
    {
...
class Symptom extends Component {
    ...
    onAddNew = () => {
        this.props.history.push('/main/my-wellness/symptom/add');
    }
    onRowClick = (rowData, index) => {
        let id = rowData.symptomsID;
        this.props.history.push(`/main/my-wellness/symptom/detail/${id}`);
    ...
    onApplyFilter = () => {
        ...
        this.props.symptomStore.findSymptoms(filter)
    }
    onClear = () => {
        this.setState({
            query: null,
    ...
    render () {
        const { totalCount, symptoms } = this.props.symptomStore;
        let data = symptoms;
        return <div className="cw-sub-wellness-container">  
            <div className="sub-header">
                <div className="sub-title">{Strings.txtSymptomList}</div>
                <div className="buttons">
                    <CButton 
                        onClick={()=>{this.onAddNew()}}
                        type="secondary"
                        containerStyle={{width: '172px', height: '40px', marginRight: '32px'}}>{Strings.txtBtnAddNewSymptom}</CButton>
                </div>
            </div>
...
export default inject('symptomStore')(observer(Symptom));
